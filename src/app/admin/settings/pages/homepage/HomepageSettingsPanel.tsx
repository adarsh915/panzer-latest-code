...
const HomepageSettingsPanel = () => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    // Validate: each slide must have a title
    const emptySlideIndex = settings.slides.findIndex((s) => !s.title.trim())
    if (emptySlideIndex !== -1) {
      toast.error(`Slide ${emptySlideIndex + 1} is missing a title (Text 2)`)
      setActiveIndex(emptySlideIndex)
      return
    }

    // Validate: if button text is set, button URL must also be set
    const badButtonIndex = settings.slides.findIndex((s) => s.buttonText.trim() && !s.buttonUrl.trim())
    if (badButtonIndex !== -1) {
      toast.error(`Slide ${badButtonIndex + 1} has a button label but no link URL`)
      setActiveIndex(badButtonIndex)
      return
    }
            ...
            <div className={styles.slideCard}>
              ...
              <div className={styles.upload}>
                {activeSlide.backgroundImage ? (
                  ...
                  <button type="button" className={styles.uploadPlaceholder} onClick={() => setShowSlideImagePickerIndex(activeIndex)}>
                    <IconifyIcon icon="tabler:photo-plus" />
                    <strong>Upload or select banner background image</strong>
                    <small>PNG, JPG, WEBP, or GIF</small>
                  </button>
                )}
                {activeSlide.backgroundImage && (
                  <button type="button" className={styles.secondaryBtn} onClick={() => setShowSlideImagePickerIndex(activeIndex)}>
            ...
            >
              ...
              <div className={styles.heroGrid}>
                <div className={styles.heroContent}>
                  ...
                  <h2 className={styles.heroTitle}><span>{activeSlide.title}</span></h2>
                ...
                <div className={styles.heroVisual}>
                  <img
                    src={activeSlide.backgroundImage || "/assets/images/hero/try-banner1-v2.png"}
                    alt={activeSlide.backgroundAlt || activeSlide.title || "Hero Banner"}
                  />
                </div>
              </div>
