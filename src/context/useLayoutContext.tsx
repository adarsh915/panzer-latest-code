...
const LayoutProvider = ({ children }: ChildrenType) => {
  ...
  // update settings
  const updateSettings = (_newSettings: Partial<LayoutState>) => setSettings((current) => ({ ...current, ..._newSettings }))
  ...
  const changeLayoutOrientation = (_newOrientation: LayoutOrientationType) => {
    updateSettings({ layoutOrientation: 'vertical' })
  }
